...
def decode_to_pil(img_item):
    """
    Robustly decode an item from HF datasets 'images' column into a PIL.Image.Image.

    Supported inputs:
      - PIL.Image.Image (already-decoded)
      - dict with keys {'bytes': binary, 'path': str} (either or both may be present)

    Returns:
      PIL.Image.Image on success, or None on failure.
    """
    # Already a PIL Image?
    ...
    if isinstance(img_item, dict):
        # Try bytes first (fast, no fs dependency)
        b = img_item.get("bytes", None)
        ...
        # Fallback to on-disk path
        p = img_item.get("path", None)
...
def build_messages(row, processor, args):
    ...
    #     images = []
    # 2) 图像部分（统一解码为 PIL）
    images_col = row.get("images", None)
    decoded_images = []
    images_input = {}

    if images_col and not args.ignore_image:
        for i, item in enumerate(images_col):
            im = decode_to_pil(item)
            if im is None:
                print(
                    f"\n[WARNING] dataset_index={row.get('dataset_index', 'N/A')}: "
                    f"image[{i}] cannot be decoded; will skip this image."
                )
                continue
            # 尺寸检查
            try:
                ...
                print(
                    f"\n[WARNING] dataset_index={row.get('dataset_index', 'N/A')}: "
                    f"image[{i}] invalid size; will skip this image."
                )
                continue
            if min(w, h) < MIN_IMAGE_DIMENSION:
                print(
                    f"\n[WARNING] Skipping dataset_index {row.get('dataset_index', 'N/A')} "
                    f"due to small image (size: {(w, h)}). "
                    f"Minimum dimension required: {MIN_IMAGE_DIMENSION}px."
                )
                # 按你之前的设计：遇到**任一**小图直接跳过整条样本
                return None
        ...
        # 如果所有图都失败，则退化为纯文本（保留样本）
        if decoded_images:
            images_input = {"multi_modal_data": {"image": decoded_images}}
        else:
            print(
                f"\n[WARNING] dataset_index={row.get('dataset_index', 'N/A')}: "
                f"all images failed to decode; fallback to text-only."
            )
    ...
    if "glm" in model_name_lower:
        ...
        if processor is not None and hasattr(processor, "apply_chat_template"):
            try:
                content = []
                if decoded_images:
                    content += [{"type": "image", "image": im} for im in decoded_images]
        ...
        if prompt_built is None:
            placeholders = _glm_image_placeholders(len(decoded_images), modality="image")
            prompt_built = _glm_wrap_chat(text_prompt, placeholders)

        # 安全检查：占位符个数是否与图片数一致（按 <|image|> 计数）
        if decoded_images:
            ph_count = prompt_built.count("<|image|>")
            if ph_count != len(decoded_images):
                print(f"[Warn] GLM placeholder({ph_count}) != images({len(decoded_images)})")
    ...
    # === 保留原有 Qwen 分支（以防切回） ===
        ...
        if decoded_images:
            vision_placeholders = "<|image_pad|>" * len(decoded_images)
            final_prompt = (
    ...
    # === 其他模型保持原样 ===
        ...
        image_placeholders = "<image>\n" * len(decoded_images) if decoded_images else ""
        final_prompt = (
